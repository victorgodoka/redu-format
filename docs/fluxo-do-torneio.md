...
- **Reset da Grand Final**: hoje **não existe**. Se quem veio do Lower Bracket vencer a Grand Final, essa vitória sozinha já é suficiente pra ser campeão - não existe uma segunda partida obrigatória pra confirmar, diferente de alguns formatos profissionais de dupla eliminação (onde o Lower Bracket vencendo a primeira Grand Final força um reset). É uma limitação conhecida e deliberada, não um esquecimento.
